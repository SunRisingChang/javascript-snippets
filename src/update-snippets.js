 ...
 * @Last Modified time: 2024-04-26 10:55:09
...
async function updateSnippets() {
  ...
  let a_tags_jsoh = $_j_s_o_h(
    "summary:contains('内置对象')+ol>li:not(:has(span))>a"
  );
  ...
  for (const a_tag_jsoh of a_tag_jsoh_array) {
    ...
    let a_tags_jsfh = $_j_s_f_h(
      "summary:contains('方法')+ol>li:not(:has(span))>a"
    );
    ...
    for (const a_tag_jsfh of a_tag_jsfh_array) {
      ...
      let fun = $_j_s_d_h(
        `#wikiArticle>h2:contains(${isHanHua ? "语法" : "Syntax"})+pre`
      ).eq(0);

      let title = fun.text().trim();
      let prefix = (
        (htmlTagNameO.length > 3
          ? htmlTagNameO.substring(0, 3)
          : htmlTagNameO) + htmlTagNameF
      )
        .toLocaleLowerCase()
        .trim();
      ...
      if (body) {
        ...
        console.log(
          chalk.white.bold(`${htmlTagNameO}-->${htmlTagNameF}--> OK!`)
        );
      }
    }
...
function getSnippetBody(fun, obj) {
  ...
  let body1 = /^[A-Z].*$/.test(snippetBodyRegArray[1])
    ? snippetBodyRegArray[1]
    : "${1:" + obj.toLocaleLowerCase() + "}";
